...
#include "linux/pantech_ssrsystem.h"
...
// p15060
    ...
    defined(CONFIG_MACH_MSM8974_EF60S) || defined(CONFIG_MACH_MSM8974_EF61K) || defined(CONFIG_MACH_MSM8974_EF62L)
    if( !strncmp(desc->name, "modem", 5) )
    {
        sys_sync();
        panic("subsys-restart: Resetting the SoC - %s crashed.", desc->name);
    }
#endif
#endif 
...
// p15060
    ...
    {
//        if( !pantech_is_usbdump_enabled() && (1 == modem_ssr_dump_enable) )
...
// p15060
#ifdef CONFIG_PANTECH_ERR_CRASH_LOGGING
        if( !strncmp( name, "modem", 5 )&& (1 == modem_ssr_dump_enable) )
        {

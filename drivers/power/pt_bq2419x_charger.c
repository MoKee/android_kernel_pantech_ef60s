...
//#define FEATURE_WORKAROUND_VBUS_COLLAPSE
...
#define IINLIM_MIN				100*1000	// 100mA
...
#if defined(FEATURE_WORKAROUND_VBUS_COLLAPSE)	
...
static int bq2419x_iinlim_set(struct pt_bq2419x_chg_chip *chip, unsigned int uA)
{
	int rc=0;
	u8 iinlim;
	unsigned int mA;
	...
	if(mA > bq2419x_iinlim_val[IINLIM_3000MA])
		mA = bq2419x_iinlim_val[IINLIM_3000MA];

	if(mA < bq2419x_iinlim_val[IINLIM_100MA])
		mA = bq2419x_iinlim_val[IINLIM_100MA];

	if(chip->dummy_batt_present)
		mA = bq2419x_iinlim_val[IINLIM_900MA];
	
	for(iinlim=IINLIM_3000MA; iinlim>=0; iinlim--) {
		if(bq2419x_iinlim_val[iinlim] <= mA)
			break;
	}
	
	printk("IN Current %dmA -> %dmA[%d]\n", mA, bq2419x_iinlim_val[iinlim], iinlim);
...
#if 0
static void bq2419x_disable_en_hiz(struct pt_bq2419x_chg_chip *chip)
{
	int rc=0;
...
static int start_auto_recharge(struct pt_bq2419x_chg_chip *chip)
{
	u8 sys_stat;
...
bq2419x_batt_external_power_changed(struct power_supply *psy)
{
	...
	for (i = 0; i < ext->num_properties; i++) {
		...
		case POWER_SUPPLY_PROP_PRESENT:
			...
			if(!chip->vbus_active) {
				chip->current_max = 0;
				bq2419x_iinlim_set(chip, chip->current_max);
			}
			else {
#if defined(FEATURE_WORKAROUND_NC_CHARGER)
...
static void bq2419x_hw_init(struct pt_bq2419x_chg_chip *chip)
{
	...
	rc = gpio_direction_output(chip->otg_gpio, 0);
	if (rc )
		pr_err("%s : error : gpio_direction_output sc_otg\n", __func__);

	// defualt USB current limit : 100mA
	gpio_set_value(chip->otg_gpio, 0);
...
update_heartbeat_work(struct work_struct *work)
{
	...
	if(get_prop_batt_present(chip)) {
		...
		if(start_auto_recharge(chip))
			bq2419x_chg_en(chip, true);
		
		printk("[SOC:%d, VCELL:%d, TEMP:%d]\n", curr_soc, 
				curr_vcell, curr_temp);	
...
#if defined(FEATURE_WORKAROUND_VBUS_COLLAPSE)
static void
bq2419x_vbus_det_work(struct work_struct *work)
{
	struct pt_bq2419x_chg_chip *chip = container_of(work,
				struct pt_bq2419x_chg_chip, vbus_det_work);

	if(gpio_get_value(chip->vin_gpio)) {
		pr_err("vbus is high\n");
	}
	else {
		pr_err("vbus is low\n");
...
bq2419x_irq_work(struct work_struct *work)
{
	...
	chg_fault = bq2419x_check_fault_status(chip);
	if(chg_fault) {
		if(chg_fault == bq2419x_check_fault_status(chip))
			pr_err("=> [Charging fault!!!] <=\n=> [09H:0x%x] <=\n", 
				chg_fault);
	}
...
#if defined(FEATURE_WORKAROUND_VBUS_COLLAPSE)	
static irqreturn_t bq2419x_vbus_det_handler(int irq, void *_chip)
{
	struct pt_bq2419x_chg_chip *chip = _chip;

	schedule_work(&chip->vbus_det_work);
	...
	case PM8941_CHARGER_TEST_CHARGING_SETTING:
		...
		if (!rc) {
			...
			if(n) {
				...
				bq2419x_iinlim_set(testmenu, IINLIM_MAX);
			}
			else{
				bq2419x_sel_pw_src(testmenu, testmenu->chg_type);
				bq2419x_iinlim_set(testmenu, testmenu->current_max);
			}
		}
		break;
...
pt_bq2419x_charger_probe(struct spmi_device *spmi)
{
	...
	printk("auto pw on : %d, dummy batt : %d\n", chip->auto_pw_on, chip->dummy_batt_present);
...
#if defined(FEATURE_WORKAROUND_VBUS_COLLAPSE)
	INIT_WORK(&chip->vbus_det_work, bq2419x_vbus_det_work);
#endif
...
#if defined(FEATURE_WORKAROUND_VBUS_COLLAPSE)	
	rc = request_irq(gpio_to_irq(chip->vin_gpio), bq2419x_vbus_det_handler, 
		(IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING), "bq2419x_vbus_det", chip);
	if(rc) {
		pr_err("%s request_threaded_irq failed for %d rc =%d\n", __func__, gpio_to_irq(chip->vin_gpio), rc);

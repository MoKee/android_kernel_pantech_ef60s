...
static void android_enable_function_cb(char *functions)
{
    ...
    while(functions){
        ...
        if(type){
            ...
            if(ptr){
                if(index > 0){
                    dev_mode_change->pc_mode_switch_flag[index++] = ':';
                }
                memcpy(&dev_mode_change->pc_mode_switch_flag[index], ptr, strlen(ptr));
                index += strlen(ptr);
            }
        }
    }//while
...
#ifdef CONFIG_PANTECH_USB_QXDM_ONOFF
static ssize_t qxdm_message_onoff_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int value;

    value = qxdm_msg_onoff_control;
    return sprintf(buf, "%d\n", value);
}

static ssize_t qxdm_message_onoff_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    int value;

    sscanf(buf, "%d", &value);
    qxdm_msg_onoff_control = value;

    return size;
}
static DEVICE_ATTR(qxdm_message_onoff, S_IRUGO | S_IWUSR, qxdm_message_onoff_show, qxdm_message_onoff_store);
#endif
//LS2_USB tarial USB3 detection test code
...
#endif
#ifdef CONFIG_PANTECH_USB_QXDM_ONOFF
    &dev_attr_qxdm_message_onoff.attr,
#endif
...
void pantech_android_set_config(struct android_configuration *a_config)
{
    ...
    dev_mode_change->g_device_type  = 0x00;
                                        ...
                                        size_t count, loff_t *ppos)
{
    ...
    if((cnt == 10) && strncmp(cmd, "manager_on", 10) == 0){
        ...
        printk(KERN_INFO "[PUSB]%s Mode change command=[%s], isFirstRun = [%d]\n", __func__, cmd, isFirstRun);
        dev_mode_change->usb_manager_on = 1;
        mutex_unlock(&_android_dev->mutex);
        return count;
    }

    printk(KERN_INFO "[PUSB]%s Mode change command=[%s]\n", __func__, cmd);
    ...
    if(strncmp(cmd, "carrier_mode", 12) == 0){
        ...
        printk(KERN_INFO "[PUSB]%s - carrier_mode [%d]\n", __func__,pantech_usb_carrier);
        return count;
    }
    ...
    if (strncmp(cmd, "gadget_off", 10) == 0) {
        printk(KERN_INFO "[PUSB]%s - Handled disconnect[%d]\n", __func__, _android_dev->enabled);
        mutex_lock(&_android_dev->mutex);
        if(_android_dev->enabled){
            android_disable(_android_dev);
            list_for_each_entry(conf, &_android_dev->configs, list_item)
    ...
    printk(KERN_INFO "[PUSB]%s - Successfully enabled function - %s \n",
           __func__, cmd);
...
/* PANTECH OBEX */
#if defined(CONFIG_PANTECH_DOMESTIC)

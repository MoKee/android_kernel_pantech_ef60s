...
#define LED_OFF_CHECK_INTERVAL 500
#endif
...
static u8 pan_boot_pattern[][LP5523_PROGRAM_LENGTH] =  {
  { 0x00, 0xc0, 0x9c, 0x00, 0x04, 0xff, 0xc0, 0x00},  // start
  { 0x00, 0x04, 0x9c, 0x00, 0x04, 0xff, 0xc0, 0x00},  // pattern 1 on
  { 0x00, 0x01, 0x9c, 0x00, 0x04, 0xff, 0xc0, 0x00},  // pattern 2 on
  { 0x00, 0x80, 0x9c, 0x10, 0x05, 0xff, 0xc0, 0x00},  // pattern 2 off
  { 0x00, 0x40, 0x9c, 0x10, 0x05, 0xff, 0xc0, 0x00},  // pattern 3 off
  { 0x00, 0x08, 0x9c, 0x00, 0x04, 0xff, 0xc0, 0x00},  // pattern 4 on
  { 0x00, 0x02, 0x9c, 0x00, 0x04, 0xff, 0xc0, 0x00},  // pattern 5 on
  { 0x00, 0x04, 0x9c, 0x10, 0x05, 0xff, 0xc0, 0x00},  // pattern 5 off
  { 0x00, 0x01, 0x9c, 0x10, 0x05, 0xff, 0xc0, 0x00},  // pattern 6 off
  { 0x00, 0x80, 0x9c, 0x00, 0x04, 0xff, 0xc0, 0x00},  // pattern 7 on
  { 0x00, 0x40, 0x9c, 0x00, 0x04, 0xff, 0xc0, 0x00},  // pattern 8 on
  { 0x00, 0x08, 0x9c, 0x10, 0x05, 0xff, 0xc0, 0x00},  // pattern 8 off
  { 0x00, 0x02, 0x9c, 0x10, 0x05, 0xff, 0xc0, 0x00},  // pattern 9 off
};
// rainbow step. 9step.
typedef enum {	
  EF60_BOOT_START_PATTERN = 0,
  EF60_BOOT_RAINBOW_PATTERN_1,
  EF60_BOOT_RAINBOW_PATTERN_2,
  EF60_BOOT_RAINBOW_PATTERN_3,
  EF60_BOOT_RAINBOW_PATTERN_4,
  EF60_BOOT_RAINBOW_PATTERN_5,
  EF60_BOOT_RAINBOW_PATTERN_6,
  EF60_BOOT_RAINBOW_PATTERN_7,
  EF60_BOOT_RAINBOW_PATTERN_8,
  EF60_BOOT_RAINBOW_PATTERN_9,
}PAN_LED_EF60_BOOT_STATE_CMD;

static int boot_led_pattern_num=0;
struct work_struct  boot_led_work;
static struct timer_list boot_led_check_timer;
#define BOOTING_RAINBOW_LED_CHECK_INTERVAL 230
static void ef60_boot_led_work(struct work_struct *work);
...
static long led_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case PAN_LED_LP5523_LED_ALL_OFF :
		...
		break;
	case PAN_LED_LP5523_LED_HOMEKEY_RED :
		for(i=0;i<PAN_NUM_LEDS;i++){
			if(i==EF59_MENU_KEY || i==EF59_BACK_KEY) continue;
			pan_led->leds[i].brightness=0;
			lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE + i,MIN_BRIGHTNESS);
		...
		break;	  
	case PAN_LED_LP5523_LED_HOMEKEY_GREEN :
		for(i=0;i<PAN_NUM_LEDS;i++){
			if(i==EF59_MENU_KEY || i==EF59_BACK_KEY) continue;
			pan_led->leds[i].brightness=0;
			lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE + i,MIN_BRIGHTNESS);
		...
		break;
	case PAN_LED_LP5523_LED_HOMEKEY_BLUE :
		for(i=0;i<PAN_NUM_LEDS;i++){
			if(i==EF59_MENU_KEY || i==EF59_BACK_KEY) continue;
			pan_led->leds[i].brightness=0;
			lp5523_write(pan_led->client, LP5523_REG_LED_PWM_BASE + i,MIN_BRIGHTNESS);
		...
		struct lp5523_platform_data *pdata )
{
	...
	/* led en gpio */	                               
	rc = of_property_read_u32(np, "lp5523,led_en_gpio", &temp_val);
	if (!rc) {		
		pdata->led_en_gpio = temp_val;
...
static void ef60_boot_led_work(struct work_struct *work)
{
  ...
  if(lp5523_write(pan_led->client, LP5523_REG_OP_MODE, LP5523_CMD_DISABLED)){
		pan_err("EF59_LED_OFF error \n");
	}
	
	// sellect pattern.
  switch(boot_led_pattern_num){
    case EF60_BOOT_START_PATTERN :
      program_engine_sel |= LP5523_CMD_RUN_ENGINE1;
      if(lp5523_load_program(&pan_led->engines[0], pan_boot_pattern[0])){
				pan_err("EF60_BOOT_START_PATTERN load error \n");
      }
      break;
    case EF60_BOOT_RAINBOW_PATTERN_1:
      program_engine_sel |= LP5523_CMD_RUN_ENGINE1;
      if(lp5523_load_program(&pan_led->engines[0], pan_boot_pattern[1])){
				pan_err("EF60_BOOT_RAINBOW_PATTERN_1 on load  error \n");
      }
      break;
    case EF60_BOOT_RAINBOW_PATTERN_2:
      program_engine_sel |= (LP5523_CMD_RUN_ENGINE1|LP5523_CMD_RUN_ENGINE2);
      if(lp5523_load_program(&pan_led->engines[0], pan_boot_pattern[2])){
				pan_err("EF60_BOOT_RAINBOW_PATTERN_1 on load error \n");
      }
      if(lp5523_load_program(&pan_led->engines[1], pan_boot_pattern[3])){
				pan_err("EF60_BOOT_RAINBOW_PATTERN_1 off load error \n");
      }
      break;
    case EF60_BOOT_RAINBOW_PATTERN_3:
      program_engine_sel |= LP5523_CMD_RUN_ENGINE2;
      if(lp5523_load_program(&pan_led->engines[1], pan_boot_pattern[4])){
				pan_err("EF60_BOOT_RAINBOW_PATTERN_1 off load error \n");
      }
      break;
    case EF60_BOOT_RAINBOW_PATTERN_4:
      program_engine_sel |= LP5523_CMD_RUN_ENGINE1;
      if(lp5523_load_program(&pan_led->engines[0], pan_boot_pattern[5])){
				pan_err("EF60_BOOT_RAINBOW_PATTERN_1 on load error \n");
      }
      break;
    case EF60_BOOT_RAINBOW_PATTERN_5:
      program_engine_sel |= (LP5523_CMD_RUN_ENGINE1|LP5523_CMD_RUN_ENGINE2);
      if(lp5523_load_program(&pan_led->engines[0], pan_boot_pattern[6])){
				pan_err("EF60_BOOT_RAINBOW_PATTERN_1 on load error \n");
      }
      if(lp5523_load_program(&pan_led->engines[1], pan_boot_pattern[7])){
				pan_err("EF60_BOOT_RAINBOW_PATTERN_1 off load error \n");
      }
      break;
    case EF60_BOOT_RAINBOW_PATTERN_6:
      program_engine_sel |= LP5523_CMD_RUN_ENGINE2;
      if(lp5523_load_program(&pan_led->engines[1], pan_boot_pattern[8])){
				pan_err("EF60_BOOT_RAINBOW_PATTERN_1 off load error \n");
      }
      break;
    case EF60_BOOT_RAINBOW_PATTERN_7:
      program_engine_sel |= LP5523_CMD_RUN_ENGINE1;
      if(lp5523_load_program(&pan_led->engines[0], pan_boot_pattern[9])){
				pan_err("EF60_BOOT_RAINBOW_PATTERN_1 on load error \n");
      }
      break;
    case EF60_BOOT_RAINBOW_PATTERN_8:
      program_engine_sel |= (LP5523_CMD_RUN_ENGINE1|LP5523_CMD_RUN_ENGINE2);
      if(lp5523_load_program(&pan_led->engines[0], pan_boot_pattern[10])){
				pan_err("EF60_BOOT_RAINBOW_PATTERN_1 on load error \n");
      }
      if(lp5523_load_program(&pan_led->engines[1], pan_boot_pattern[11])){
				pan_err("EF60_BOOT_RAINBOW_PATTERN_1 off load error \n");
      }
      break;
    case EF60_BOOT_RAINBOW_PATTERN_9:
      program_engine_sel |= LP5523_CMD_RUN_ENGINE2;
      if(lp5523_load_program(&pan_led->engines[1], pan_boot_pattern[12])){
				pan_err("EF60_BOOT_RAINBOW_PATTERN_1 off load error \n");
      }
      break;
    default :
      pan_err("ef60_boot_led_work boot_led_pattern_num is error[%d]\n",boot_led_pattern_num);
      break;    
  }
  
  // Pattern number is increasing for next pattern
  if(boot_led_pattern_num < EF60_BOOT_RAINBOW_PATTERN_9){
    boot_led_pattern_num++;
  }else{
    boot_led_pattern_num = EF60_BOOT_RAINBOW_PATTERN_1;
  }
  
  // engine program register set.
	if(lp5523_write(pan_led->client, LP5523_REG_ENABLE,	program_engine_sel | LP5523_ENABLE)){
	  pan_err("ef60_boot_led_work program reg enable is error\n");	  
	}
	
	// engine program excute.
	if( lp5523_write(pan_led->client, LP5523_REG_OP_MODE, program_engine_sel)){
    pan_err("ef60_boot_led_work program reg mode is error\n");
	} 
	// next timer set.
		...
		const struct i2c_device_id *id)
{
  ...
  if(!pan_led_single_thread_queue){
    ...
    INIT_WORK(&boot_led_work,ef60_boot_led_work);

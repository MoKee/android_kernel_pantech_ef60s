...
void set_finger_cover_state(int state)
{
	...
	DBG_CR("fpc1080_probe_plug is called!\n");
	if(gpio_get_value(GPIO_BTP_ATTACH))
	{
		FPC1080_PLUG = 0;
		DBG_CR("fpc1080_probe_plug ::FPC1080_PLUG = %d, FPC1080_PLUG_PROBE = %d. \n",FPC1080_PLUG, FPC1080_PLUG_PROBE);
	}
	else
	{
		FPC1080_PLUG = 1;
		...
		if((FPC1080_PLUG == 1) && (FPC1080_PLUG_PROBE == 0))
		{
			...
			DBG_CR("fpc1080_probe_plug is called with %d, addr_g : %p.\n",error,fpc1080_g);
		}
	}
	if((state == 1)&&(FPC1080_PLUG == 0))
	{
		DBG_CR("set_finger_cover_state ::HallICstate = %d, FPC1080_PLUG = %d. \n",state, FPC1080_PLUG);
	...
	{
		DBG_CR("set_finger_cover_state ::HallICstate = %d, FPC1080_PLUG = %d, FPC1080_PLUG_PROBE = %d. \n",state, FPC1080_PLUG,FPC1080_PLUG_PROBE);
...
void set_finger_cover_state2(int state)
{
	...
	if((state == 1)&&(FPC1080_PLUG == 0))
	{
		DBG_CR("set_finger_cover_state2 ::HallICstate = %d, FPC1080_PLUG = %d. \n",state, FPC1080_PLUG);
		envp[0] = fpc1080_state_unusable;
		kobject_uevent_env(&fpc1080_g->device->kobj, KOBJ_CHANGE, envp);
	}
	if((state == 0)&&(FPC1080_PLUG == 1)&&(FPC1080_PLUG_PROBE == 1))
	{
		DBG_CR("set_finger_cover_state2 ::HallICstate = %d, FPC1080_PLUG = %d, FPC1080_PLUG_PROBE = %d. \n",state, FPC1080_PLUG,FPC1080_PLUG_PROBE);
		envp[0] = fpc1080_state_usable;
		kobject_uevent_env(&fpc1080_g->device->kobj, KOBJ_CHANGE, envp);
	}
...
{ 
	...
	if(gpio_get_value(BUMPER_CASE)){
		printk(" BUMPER_CASE is High\n");
  	}else{
    		printk(" BUMPER_CASE is Low\n");
  	}
...
/*ATTACH PIN HANDLER*/
	...
	if (!gpio_attach) {
		...
		gpio_attach->debounce_time = ktime_set(0, 2000000000); //10ms : 10000000 // 2sec : 2000000000

...
#if (defined(CONFIG_MACH_MSM8974_EF60S) || defined(CONFIG_MACH_MSM8974_EF61K) || defined(CONFIG_MACH_MSM8974_EF62L)) //p11774 GPIO re-config solution
...
#endif
struct spi_device *btp_spi_dev = NULL;
/* -------------------------------------------------------------------- */
/* fpc1080 data types							*/
/* -------------------------------------------------------------------- */
						...
						u8 value, unsigned int size)
{
	...
	if(error)
	{
		DBG_CR("spi_timeout count = %d", spi_timeout);
		//panic("Kernel_reset!\n");
		return error;
	}
#else 
